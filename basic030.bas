...
451 select case cmd
  ...
  case token_close     	: compile_fun_1p()                    'no params, do nothing, only add a command to the line, but case needs something to do after 
  ...
  case token_enter	: vars=compile_fun_varp() 

  case token_get	: vars=compile_fun_varp()
  ...
  case token_put	: vars=compile_fun_varp()
...
psdpoke arrayptr,arraytype ': print arraytype, arrayptr
psdpoke arrayptr+2,esize ': print esize , psdpeek(arrayptr+2)
pslpoke arrayptr+4,dims(0)
pslpoke arrayptr+8,dims(1)
pslpoke arrayptr+12,dims(2)
'for i=0 to 15: print hex$(pspeek(arrayptr+i),2), : next i
'for i=0 to 3: print hex$(pslpeek(arrayptr+4*i),2), : next i
